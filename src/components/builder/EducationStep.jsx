  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <div className="col-span-2">
        <label className="block text-xs font-medium text-text-secondary mb-1">Degree / Qualification</label>
      ...
      <div>
        <label className="block text-xs font-medium text-text-secondary mb-1">Start Year</label>
        <input className="input-field" placeholder="2018" value={data.startYear || ''} onChange={e => set('startYear', e.target.value)} />
      </div>
      <div>
        <label className="block text-xs font-medium text-text-secondary mb-1">End Year</label>
...
function EducationPreview({ item }) {
  return (
    <div>
      ...
      <div className="text-xs text-text-secondary mt-0.5">{item.startYear}{item.endYear ? ` – ${item.endYear}` : ''}</div>
    </div>
  )
}
